...
namespace Helpers {
	...
	istream& operator >> (istream& is, Agent& a) {
		...
		input_int(is, "Enter Level(max 9):", "Level", a.level);
	...
	ifstream& operator >> (ifstream& is, Criminal& c) {
		...
		for (int i = 0; i < c.number_of_cases; ++i)input_line(is, c.case_charged.para, 100);
	...
	ofstream& operator << (ofstream& os, Criminal& c) {
		...
		for (int i = 0; i < c.number_of_cases; ++i)output_line(os, c.case_charged.para, 100);
	...
	void login_session(int start) {
		...
		//out << endl << "Session: ";
		if (mins > 0)
			output_num(out, mins, 69, 2);
		output_num(out, secs, 69, 2);
	}   
	/*void see_log() {
		ifstream in("LOG");
		string s;
